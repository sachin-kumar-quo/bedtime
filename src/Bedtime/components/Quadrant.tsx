...
const Quadrant = ({ sufficientSleep }) => {
  return (
    <>
      <Circle
        strokeWidth={STROKE}
      ...
      />
      <G mask="url(#mask)">
        <Circle fill={sufficientSleep? 'grey' :"#FD9F07"} cx={SIZE / 2} cy={SIZE / 2} r={R + PADDING} />
      ...
      {new Array(24).fill(0).map((_, i) => {
        ...
        const p1 = polar2Canvas({ theta, radius: R - PADDING }, CENTER);
          ...
          <React.Fragment key={i}>
            ...
            {new Array(4).fill(0).map((_e, j) => {
              ...
              const s = polar2Canvas(
                { theta: alpha, radius: R - 1.3 * PADDING },
                CENTER
              );
              const e = polar2Canvas(
                { theta: alpha, radius: R - (2 * PADDING) / 2 },
                CENTER
              );
              return (
                <Line
